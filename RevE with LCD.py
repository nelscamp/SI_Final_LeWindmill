...
class MotorControlSystem:
    def __init__(self, log_queue):
        ...
        self.music_speed_factor = 2.0  # Adjust as needed
...
class WindmillControlGUI(ctk.CTk):
    def __init__(self):
        ...
        self.after(1000, self.update_lcd_display)
    ...
    def update_lcd_display(self):
        ...
        try:
            ...
            # Update LCD content
            self.lcd.clear()
            self.lcd.write_string(f"RPM: {rpm}")
            self.lcd.cursor_pos = (1, 0)
            self.lcd.write_string(f"Status: {status}")

        except Exception as e:
            self.log_to_terminal(f"LCD Update Error: {e}")
    ...
    def update_status_label(self):
        ...
        # Optionally, you can change the text color in the GUI based on status
        # For example:
